"""
ViSoBERT Embedding Extraction Module

Trích xuất embeddings từ:
- ViSoBERT pre-trained (uitnlp/visobert)
- ViSoBERT fine-tuned (models/bert_embedding/visobert-finetuned)
...
MODEL_CONFIG = {
    ...
    'finetuned_path': 'models/bert_embedding/visobert-finetuned',
    'max_length': 256,
    'batch_size': 16,
    'hidden_size': 768,
}
...
# =============================================================================
# VISOBERT ENCODER CLASS
# =============================================================================

class ViSoBertEncoder:
    """
    ViSoBERT Embedding Extractor
    
    Hỗ trợ cả pre-trained và fine-tuned models
    
    Usage:
        # Pre-trained
        encoder = ViSoBertEncoder(use_finetuned=False)
        embeddings = encoder.extract(texts)
        
        # Fine-tuned
        encoder = ViSoBertEncoder(use_finetuned=True)
        embeddings = encoder.extract(texts)
    """
    ...
    def __init__(self, 
        ...
        if use_finetuned:
            ...
            self.model_name = "ViSoBERT (fine-tuned)"
        else:
            self.model_path = MODEL_CONFIG['pretrained_name']
            self.model_name = "ViSoBERT (pre-trained)"
        
        print("="*60)
        print(f"ViSoBERT Encoder: {self.model_name}")
        print("="*60)
    ...
    def _load_model(self):
        ...
        if self.use_finetuned:
            ...
            # ViSoBERT dùng XLMRobertaTokenizer
            ...
            # ViSoBERT dùng XLM-RoBERTa architecture
            if hasattr(full_model, 'roberta'):
                self.model = full_model.roberta
            elif hasattr(full_model, 'xlm_roberta'):
                self.model = full_model.xlm_roberta
...
def create_visobert_embeddings(data_path: str,
                                output_dir: str,
                                text_col: str = 'text_bert',
                                id_col: str = 'id',
    ...
    """
    print("\n" + "="*60)
    print("CREATING VISOBERT EMBEDDINGS")
    print("="*60)
    ...
    print("\n" + "-"*40)
    print("1. Pre-trained ViSoBERT")
    print("-"*40)
    encoder_pretrained = ViSoBertEncoder(use_finetuned=False, batch_size=batch_size)
    ...
    pretrained_path = visobert_dir / "pretrained.npz"
    ...
    print("\n" + "-"*40)
    print("2. Fine-tuned ViSoBERT")
    print("-"*40)
    encoder_finetuned = ViSoBertEncoder(use_finetuned=True, batch_size=batch_size)
    emb_finetuned = encoder_finetuned.extract(texts)
    
    finetuned_path = visobert_dir / "finetuned.npz"
    ...
    print("\n" + "="*60)
    print("VISOBERT EMBEDDINGS CREATED!")
    print("="*60)
...
if __name__ == "__main__":
    ...
    # Create embeddings
    result_pre, result_ft = create_visobert_embeddings(
