"""
PhoBERT v2 Embedding Extraction Module

Trích xuất embeddings từ:
- PhoBERT v2 pre-trained (vinai/phobert-base-v2)
- PhoBERT v2 fine-tuned (models/bert_embedding/phobert-v2-finetuned)
...
MODEL_CONFIG = {
    ...
    'finetuned_path': 'models/bert_embedding/phobert-v2-finetuned',
    'max_length': 256,
    'batch_size': 16,
    'hidden_size': 768,
}
...
# =============================================================================
# PHOBERT V2 ENCODER CLASS
# =============================================================================

class PhoBertV2Encoder:
    """
    PhoBERT v2 Embedding Extractor
    
    Hỗ trợ cả pre-trained và fine-tuned models
    
    Usage:
        # Pre-trained
        encoder = PhoBertV2Encoder(use_finetuned=False)
        embeddings = encoder.extract(texts)
        
        # Fine-tuned
        encoder = PhoBertV2Encoder(use_finetuned=True)
        embeddings = encoder.extract(texts)
    """
    ...
    def __init__(self, 
        ...
        if use_finetuned:
            ...
            self.model_name = "PhoBERT v2 (fine-tuned)"
        else:
            self.model_path = MODEL_CONFIG['pretrained_name']
            self.model_name = "PhoBERT v2 (pre-trained)"
        
        print("="*60)
        print(f"PhoBERT v2 Encoder: {self.model_name}")
        print("="*60)
...
def create_phobertv2_embeddings(data_path: str,
                                 output_dir: str,
                                 text_col: str = 'text_bert',
                                 id_col: str = 'id',
    ...
    """
    print("\n" + "="*60)
    print("CREATING PHOBERT V2 EMBEDDINGS")
    print("="*60)
    ...
    print("\n" + "-"*40)
    print("1. Pre-trained PhoBERT v2")
    print("-"*40)
    encoder_pretrained = PhoBertV2Encoder(use_finetuned=False, batch_size=batch_size)
    ...
    pretrained_path = phobertv2_dir / "pretrained.npz"
    ...
    print("\n" + "-"*40)
    print("2. Fine-tuned PhoBERT v2")
    print("-"*40)
    encoder_finetuned = PhoBertV2Encoder(use_finetuned=True, batch_size=batch_size)
    emb_finetuned = encoder_finetuned.extract(texts)
    
    finetuned_path = phobertv2_dir / "finetuned.npz"
    ...
    print("\n" + "="*60)
    print("PHOBERT V2 EMBEDDINGS CREATED!")
    print("="*60)
...
if __name__ == "__main__":
    ...
    # Create embeddings
    result_pre, result_ft = create_phobertv2_embeddings(
